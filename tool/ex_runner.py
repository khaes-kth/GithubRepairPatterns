...
def process(repoName, repoUrl, branch, tool):
    ...
    os.system(
        f"java -Xmx1g -classpath ./coming.jar fr.inria.coming.main.ComingMain -location ./{exec_id}/ -mode repairability -repairtool {tool} -output ./{exec_id}_all_out -filter numberhunks:maxfiles:maxfilesize -parameters include_all_instances_for_each_tool:true:executed_by_travis:true:exclude_repair_patterns_not_covering_the_whole_diff:false:maxrevision:500:save_result_revision_analysis:false:output_for_repair:true:max_commit_analysis_time:600000:file_complete_name:true:forbidden_filename_substrings:test:branch:{branch}:CLASSES_HIERARCHY_FILE_NAME:/home/khaes/phd/projects/LighteR/tmp/gumtree-inheritance-relations.txt:MAX_ALL_OPERATIONS_PER_FILE:500:max_files_per_commit:100 1> ./{exec_id}_all_out/stdout.txt 2> ./{exec_id}_all_out/stderr.txt")
    os.system(f"rm {exec_id} -rf")

    os.system(f"git clone {repoUrl} {exec_id}")
    os.system(
        f"java -Xmx1g -classpath ./coming.jar fr.inria.coming.main.ComingMain -location ./{exec_id}/ -mode repairability -repairtool {tool} -output ./{exec_id}_filtered_out -filter numberhunks:maxfiles:maxfilesize -parameters include_all_instances_for_each_tool:true:executed_by_travis:true:exclude_repair_patterns_not_covering_the_whole_diff:true:maxrevision:500:save_result_revision_analysis:false:output_for_repair:true:max_commit_analysis_time:600000:file_complete_name:true:forbidden_filename_substrings:test:branch:{branch}:CLASSES_HIERARCHY_FILE_NAME:/home/khaes/phd/projects/LighteR/tmp/gumtree-inheritance-relations.txt:MAX_ALL_OPERATIONS_PER_FILE:500:max_files_per_commit:100 1> ./{exec_id}_filtered_out/stdout.txt 2> ./{exec_id}_filtered_out/stderr.txt")
    os.system(f"rm {exec_id} -rf")
